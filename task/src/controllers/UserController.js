...
const formatUser = (title) => {
    const formatedName = users.map((result) => Object.create({name:title+" "+result.name, age:result.age, major:result.major }));
...
const filterByMajor = (major) => {
    const filterUserByMajor = users.filter((user) => user.major == major );
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            resolve(filterUserByMajor);
        },4000);
    })
};
